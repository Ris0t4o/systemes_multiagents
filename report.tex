...
channel between hiders; and a cover-seeking primitive that picks a point
geometrically behind an obstacle relative to the last-known seeker pose. The
overall control law combines a Lissajous roaming target, a consensus-based
obstacle push, inter-drone separation, and soft boundary repulsion, with a
single magnitude saturation at the actuator limit.
\end{abstract}
...
\code{tp\_algos.py} & The required entry-point file (prof requirement). The
controller stubs there delegate to the modules below. \emph{Edits kept
minimal.} \\
\code{cf2\_milestone1.py} & Top-level hider controller: composes a goal force,
obstacle consensus, inter-drone separation, and boundary repulsion; switches
between roaming and cover-seeking. \\
...
\code{rmtt\_seeker.py} & Seeker controller and the \code{can\_see} predicate
shared with the referee and the hiders' seeker-detection logic. \\
...
\begin{enumerate}[leftmargin=1.4em]
  ...
  \item Submits each robot's controller to a thread pool. The controller
        returns a target velocity (and for drones, a target altitude / takeoff
        / land flags).
...
\subsubsection*{Inter-drone separation}
...
(2D). The shape vanishes smoothly at $d = d_0$ and grows unbounded as $d \to 0$
-- the same philosophy as the obstacle barrier, but between drones.
...
\[
    ...
    \end{pmatrix},
\]
with $A_x = 1.6$, $A_y = 3.0$, $c_z = 1.0$, and per-drone angular frequencies
$\omega_x^{(i)} = 0.16 + 0.018(i-1)$ rad/s (analogous for $y, z$), and coprime
phase offsets $\varphi^{(i)}_x = (i-1)\cdot 2\pi/3$, $\varphi^{(i)}_y = (i-1)\cdot 2\pi/5$,
$\varphi^{(i)}_z = (i-1)\cdot 2\pi/7$. The amplitudes are chosen so the path
sits inside the boundary-repulsion margin, and the per-drone variation in
$\omega$ prevents the swarm from collapsing onto a single shared orbit.

The $z$-component is currently held constant at $c_z$; a slow vertical wobble
is available via $A_z = 0.5$ but is commented out so the FoV cones stay at a
predictable altitude in the visualization.
...
hider positions: structural enforcement of the rule that the seeker has no
knowledge of where the hiders are.
...
% ============================================================
\section{Simulator hooks (\code{tp\_algos.py})}
% ============================================================

The TP framework requires every controller to live in \code{tp\_algos.py} with
a fixed signature. Per the project rules, only the body marked
\code{TO BE MODIFIED} may be edited. The implementation there is a thin shim:
\begin{itemize}[leftmargin=1.4em]
  \item \code{cf2\_control\_fn}: runs the takeoff state machine for the
        hider, queries \code{game\_referee.is\_caught} to decide whether to
        land, and otherwise delegates to
        \code{cf2\_milestone1.compute\_roaming\_cmd}.
  \item \code{rmtt\_control\_fn}: delegates straight to
        \code{rmtt\_seeker.compute\_seeker\_cmd} (deliberately \emph{not}
        passing \code{cf2\_poses}, so the structural ``the seeker doesn't
        know where the hiders are'' property is preserved).
  \item \code{tb3B\_control\_fn}, \code{tb3W\_control\_fn},
        \code{rmep\_control\_fn}: placeholder behaviour (a simple goal seek);
        these robots are not part of the hide-and-seek game in the current
        configuration.
\end{itemize}

The lazy-import trick (\code{if not hasattr(\dots, "\_cached"): import \dots})
keeps the file fast to reload and avoids polluting the module's namespace
beyond what the framework expects.
...
$d_0$ (sep) & 0.9 m & Inter-hider separation influence radius. \\
$g_\mathrm{sep}$ & 5 & Inter-hider separation gain. \\
...
$\omega_x^{(i)}$ etc. & $0.16 + 0.018(i-1)$ rad/s & Hider Lissajous (per-drone). \\
$A_x, A_y, A_z$ & 1.6, 3.0, 0.5 m & Hider Lissajous amplitudes. \\
...
\paragraph{Why structurally hide the hider positions from the seeker.}
The seeker controller's signature does not accept \code{cf2\_poses}, and the
referee module is the only place that knows about both. This guarantees the
seeker cannot ``peek'' even by accident -- any future modification to the
seeker controller would have to explicitly change its signature, which would
be caught in review.
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{Per-hider altitude diversity.} Re-enabling the $A_z$ term in
        the Lissajous would spread the hiders vertically, making the seeker's
        narrow cone less effective.
...
\code{tp\_algos.py}       & $\sim$ 300 & Required entry-point shim. \\
